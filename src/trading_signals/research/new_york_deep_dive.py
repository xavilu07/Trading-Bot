...
def analyze_new_york_deep_dive(*, data_path: Path, now: datetime | None = None) -> dict[str, Any]:
    ...
    rows = [row for row in baseline if _session(row) == TARGET_SESSION]
...
def write_new_york_deep_dive_report(result: dict[str, Any], reports_path: Path) -> Path:
    reports_path.mkdir(parents=True, exist_ok=True)
    path = reports_path / "new_york_deep_dive.md"
    path.write_text(format_new_york_deep_dive_markdown(result), encoding="utf-8")
    return path


def format_new_york_deep_dive_markdown(result: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- NEW_YORK: {_metrics_inline(result.get('metrics', {}))}",
        f"- Is NEW_YORK globally toxic? {answers.get('globally_toxic', '')}",
        ...
        *_rank_table(result.get("survivors", [])),
        ...
        f"- PF without NEW_YORK: {counterfactual.get('without_new_york_metrics', {}).get('profit_factor', 0)}",
        f"- TotalR current: {counterfactual.get('current_metrics', {}).get('total_r', 0)}",
        f"- TotalR without NEW_YORK: {counterfactual.get('without_new_york_metrics', {}).get('total_r', 0)}",
...
def _answers(
    *,
    ...
    without_pf = _pf_float(counterfactual.get("without_new_york_metrics", {}).get("profit_factor"))
...
def _counterfactual_remove_new_york(baseline: list[dict[str, Any]]) -> dict[str, Any]:
    removed = [row for row in baseline if _session(row) == TARGET_SESSION]
    without = [row for row in baseline if _session(row) != TARGET_SESSION]
    current = _metrics(baseline)
    without_metrics = _metrics(without)
    return {
        "current_metrics": current,
        "without_new_york_metrics": without_metrics,
