...
TARGET_REASON = "body_ratio_below_threshold"
MIN_SUBGROUP_TRADES = 5
MIN_CRITICAL_TRADES = 10


def analyze_body_ratio_below_threshold_deep_dive(*, data_path: Path, now: datetime | None = None) -> dict[str, Any]:
    ...
    rows = [row for row in baseline if TARGET_REASON in _tokens(row.get("rejection_reasons"))]
    ...
    counterfactual = _counterfactual_remove_body_ratio(baseline)
    answers = _answers(metrics=metrics, toxic=toxic, survivors=survivors, counterfactual=counterfactual)
    return {
        "scope": "BODY_RATIO_BELOW_THRESHOLD_DEEP_DIVE",
...
def write_body_ratio_below_threshold_deep_dive_report(result: dict[str, Any], reports_path: Path) -> Path:
    reports_path.mkdir(parents=True, exist_ok=True)
    path = reports_path / "body_ratio_below_threshold_deep_dive.md"
    path.write_text(format_body_ratio_below_threshold_deep_dive_markdown(result), encoding="utf-8")
    return path


def format_body_ratio_below_threshold_deep_dive_markdown(result: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- body_ratio_below_threshold: {_metrics_inline(result.get('metrics', {}))}",
        ...
        *_rank_table(result.get("survivors", [])),
        ...
        f"- PF without body_ratio_below_threshold: {counterfactual.get('without_body_ratio_metrics', {}).get('profit_factor', 0)}",
        f"- TotalR current: {counterfactual.get('current_metrics', {}).get('total_r', 0)}",
        f"- TotalR without body_ratio_below_threshold: {counterfactual.get('without_body_ratio_metrics', {}).get('total_r', 0)}",
...
def _answers(
    *,
    ...
    without_pf = _pf_float(counterfactual.get("without_body_ratio_metrics", {}).get("profit_factor"))
...
def _counterfactual_remove_body_ratio(baseline: list[dict[str, Any]]) -> dict[str, Any]:
    without = [row for row in baseline if TARGET_REASON not in _tokens(row.get("rejection_reasons"))]
    current = _metrics(baseline)
    without_metrics = _metrics(without)
    return {
        "current_metrics": current,
        "without_body_ratio_metrics": without_metrics,
        "removed_metrics": _metrics([row for row in baseline if TARGET_REASON in _tokens(row.get("rejection_reasons"))]),
