...
def analyze_new_york_ranging_deep_dive(*, data_path: Path, now: datetime | None = None) -> dict[str, Any]:
    now_dt = _aware(now or datetime.now(tz=UTC))
    all_trades = load_canonical_closed_trades(data_path)
    baseline = [row for row in all_trades if not _is_bullish_sweep(row) and not _is_against_htf_breakout(row)]
    rows = [row for row in baseline if _session(row) == TARGET_SESSION and _market_regime(row) == TARGET_REGIME]
    ...
    counterfactual = _counterfactual_remove_new_york_ranging(baseline)
    answers = _answers(metrics=metrics, toxic=toxic, survivors=survivors, counterfactual=counterfactual)
    return {
        "scope": "NEW_YORK_RANGING_DEEP_DIVE",
...
def write_new_york_ranging_deep_dive_report(result: dict[str, Any], reports_path: Path) -> Path:
    reports_path.mkdir(parents=True, exist_ok=True)
    path = reports_path / "new_york_ranging_deep_dive.md"
    path.write_text(format_new_york_ranging_deep_dive_markdown(result), encoding="utf-8")
    return path


def format_new_york_ranging_deep_dive_markdown(result: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "## Executive Summary",
        "",
        f"- Baseline after active blocks: {_metrics_inline(result.get('baseline_metrics', {}))}",
        f"- NEW_YORK + RANGING: {_metrics_inline(result.get('metrics', {}))}",
        f"- Is NEW_YORK + RANGING globally toxic? {answers.get('globally_toxic', '')}",
        ...
        *_rank_table(result.get("survivors", [])),
        ...
        "",
        f"- PF current after active blocks: {counterfactual.get('current_metrics', {}).get('profit_factor', 0)}",
        f"- PF without NEW_YORK_RANGING: {counterfactual.get('without_new_york_ranging_metrics', {}).get('profit_factor', 0)}",
        f"- TotalR current: {counterfactual.get('current_metrics', {}).get('total_r', 0)}",
        f"- TotalR without NEW_YORK_RANGING: {counterfactual.get('without_new_york_ranging_metrics', {}).get('total_r', 0)}",
...
def _answers(
    *,
    ...
    without_pf = _pf_float(counterfactual.get("without_new_york_ranging_metrics", {}).get("profit_factor"))
...
def _counterfactual_remove_new_york_ranging(baseline: list[dict[str, Any]]) -> dict[str, Any]:
    removed = [row for row in baseline if _session(row) == TARGET_SESSION and _market_regime(row) == TARGET_REGIME]
    without = [row for row in baseline if not (_session(row) == TARGET_SESSION and _market_regime(row) == TARGET_REGIME)]
    current = _metrics(baseline)
    without_metrics = _metrics(without)
    return {
        "current_metrics": current,
        "without_new_york_ranging_metrics": without_metrics,
