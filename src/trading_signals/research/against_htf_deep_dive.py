...
def analyze_against_htf_deep_dive(*, data_path: Path, now: datetime | None = None) -> dict[str, Any]:
    ...
    rows = [row for row in baseline if _is_against_htf(row)]
...
def write_against_htf_deep_dive_report(result: dict[str, Any], reports_path: Path) -> Path:
    reports_path.mkdir(parents=True, exist_ok=True)
    path = reports_path / "against_htf_deep_dive.md"
    path.write_text(format_against_htf_deep_dive_markdown(result), encoding="utf-8")
    return path


def format_against_htf_deep_dive_markdown(result: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "## Executive Summary",
        "",
        f"- Baseline without bullish_sweep: {_metrics_inline(result.get('baseline_metrics', {}))}",
        f"- against_htf: {_metrics_inline(result.get('metrics', {}))}",
        ...
        *_rank_table(result.get("survivors", [])),
        ...
        f"- PF without against_htf: {counterfactual.get('without_against_htf_metrics', {}).get('profit_factor', 0)}",
        f"- TotalR current: {counterfactual.get('current_metrics', {}).get('total_r', 0)}",
        f"- TotalR without against_htf: {counterfactual.get('without_against_htf_metrics', {}).get('total_r', 0)}",
    ...
    for title, key in (
        ...
        ("By Penalty", "penalty"),
        ("By Other Warning", "other_warning"),
    ):
...
def _answers(
    *,
    ...
    without_pf = _pf_float(counterfactual.get("without_against_htf_metrics", {}).get("profit_factor"))
...
def _counterfactual_remove_against_htf(baseline: list[dict[str, Any]]) -> dict[str, Any]:
    removed = [row for row in baseline if _is_against_htf(row)]
    without = [row for row in baseline if not _is_against_htf(row)]
    current = _metrics(baseline)
    without_metrics = _metrics(without)
    return {
        "current_metrics": current,
        "without_against_htf_metrics": without_metrics,
...
def _token_summary(rows: list[dict[str, Any]], *fields: str, exclude: set[str] | None = None) -> dict[str, dict[str, Any]]:
...
def _is_against_htf(row: dict[str, Any]) -> bool:
    warning_tokens = _tokens(row.get("warnings")) | _tokens(row.get("avoidance_warnings"))
    reason_tokens = _tokens(row.get("rejection_reasons")) | _tokens(row.get("penalties"))
    return TARGET_REASON in warning_tokens or TARGET_REASON in reason_tokens or _htf_alignment(row) == TARGET_REASON
