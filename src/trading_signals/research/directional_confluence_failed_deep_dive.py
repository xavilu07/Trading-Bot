...
MIN_SUBGROUP_TRADES = 5
MIN_CRITICAL_TRADES = 30


def analyze_directional_confluence_failed_deep_dive(*, data_path: Path, now: datetime | None = None) -> dict[str, Any]:
    now_dt = _aware(now or datetime.now(tz=UTC))
    all_trades = load_canonical_closed_trades(data_path)
    rows = [row for row in all_trades if _has_target_reason(row)]
    ...
    counterfactual = _counterfactual_remove_directional_confluence(all_trades)
    impact = _impact_ranking(rows, groups)
...
def write_directional_confluence_failed_deep_dive_report(result: dict[str, Any], reports_path: Path) -> Path:
    reports_path.mkdir(parents=True, exist_ok=True)
    path = reports_path / "directional_confluence_failed_deep_dive.md"
    path.write_text(format_directional_confluence_failed_deep_dive_markdown(result), encoding="utf-8")
    return path


def format_directional_confluence_failed_deep_dive_markdown(result: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- directional_confluence_failed: {_metrics_inline(result.get('metrics', {}))}",
        f"- Is globally toxic? {answers.get('globally_toxic', '')}",
        f"- Biggest loss contributor: {answers.get('main_loss_subgroup', '')}",
        f"- Best survivor: {answers.get('survivor_subgroup', '')}",
        f"- Would removal improve PF materially? {answers.get('material_pf_improvement', '')}",
        ...
        *_rank_table(result.get("survivors", [])),
        ...
        f"- PF without directional_confluence_failed: {counterfactual.get('without_directional_confluence_metrics', {}).get('profit_factor', 0)}",
        f"- TotalR current: {counterfactual.get('current_metrics', {}).get('total_r', 0)}",
        f"- TotalR without directional_confluence_failed: {counterfactual.get('without_directional_confluence_metrics', {}).get('total_r', 0)}",
        f"- Winrate delta: {counterfactual.get('winrate_delta', 0)}",
        f"- Trades removed: {counterfactual.get('trades_removed', 0)}",
        "",
        "## Impact Ranking",
        "",
        *_impact_table(result.get("impact_ranking", [])),
        "",
        "## Breakdowns",
        "",
    ]
...
def _answers(
    *,
    ...
    without_pf = _pf_float(counterfactual.get("without_directional_confluence_metrics", {}).get("profit_factor"))
...
def _counterfactual_remove_directional_confluence(all_trades: list[dict[str, Any]]) -> dict[str, Any]:
    removed = [row for row in all_trades if _has_target_reason(row)]
    without = [row for row in all_trades if not _has_target_reason(row)]
    current = _metrics(all_trades)
    without_metrics = _metrics(without)
    return {
        "current_metrics": current,
        "without_directional_confluence_metrics": without_metrics,
...
def _impact_ranking(rows: list[dict[str, Any]], groups: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    return sorted(results, key=lambda row: (float(row["r_improvement"]), float(row["pf_improvement"]), int(row["trades_removed"])), reverse=True)[:30]
...
def _belongs_to_group(row: dict[str, Any], dimension: str, value: str) -> bool:
    if dimension in {"warning", "penalty", "rejection_reason"}:
        ...
        return any(value in _tokens(row.get(field)) for field in fields)
...
def _has_target_reason(row: dict[str, Any]) -> bool:
    return TARGET_REASON in (
...
def _impact_table(rows: object) -> list[str]:
    lines = [
