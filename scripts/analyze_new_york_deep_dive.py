...
from trading_signals.research.new_york_deep_dive import (  # noqa: E402
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(prog="new-york-deep-dive")
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = analyze_new_york_deep_dive(data_path=Path(args.data_path))
    report_path = write_new_york_deep_dive_report(result, Path(args.reports_path))
    metrics = result["metrics"]
    counterfactual = result["counterfactual_removal"]
    print("NEW_YORK_DEEP_DIVE")
    print(f"- Trades: {metrics['trades']}")
    print(f"- WR: {metrics['winrate']}%")
    print(f"- PF: {metrics['profit_factor']}")
    print(f"- Total R: {metrics['total_r']}")
    print(f"- Classification: {result['classification']}")
    print(f"- PF without NEW_YORK: {counterfactual['without_new_york_metrics']['profit_factor']}")
