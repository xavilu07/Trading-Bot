...
TARGET_REASON = "quality_score_failed"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(prog="quality-score-failed-deep-dive")
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("QUALITY_SCORE_FAILED_DEEP_DIVE")
...
def analyze(*, data_path: Path, reports_path: Path, logs_path: Path, top: int = 12) -> dict[str, Any]:
    ...
    result = {
        "scope": "QUALITY_SCORE_FAILED_DEEP_DIVE",
...
def write_reports(result: dict[str, Any], reports_path: Path) -> dict[str, Path]:
    ...
    markdown = reports_path / "quality_score_failed_deep_dive.md"
    json_path = reports_path / "quality_score_failed_deep_dive.json"
...
def _code_origin() -> dict[str, Any]:
    return {
        "file": CODE_PATH,
        "approx_lines": "evaluate(): final score check around lines 188-203",
        "conditions": [
        ...
        "hard_veto_behavior": (
            "`quality_score_failed` is included in the hard-failure set together with volatility/body/late-entry/pullback gates. "
            "When present, the strategy skips all primary, secondary, and fallback directional signal branches."
        ),
    }
...
def _severe_cases(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    directional_passed = matching(lambda row: "directional_confluence" in set(row.get("conditions_passed", [])) and TARGET_REASON in set(row.get("conditions_failed", []) + row.get("rejection_reasons", [])))
...
def _conclusion(severe: dict[str, Any], evidence: dict[str, Any]) -> dict[str, str]:
    ...
    score70_count = int(severe["score_gte_70"]["count"])
    if closed < 10:
        return {
    ...
    if total_r > 0 and pf >= 1.2 and score70_count >= 5:
        return {
            "evidence_classification": "BLOCKING_EDGE",
            "recommended_action": "crear shadow relaxation",
            "rationale": "Hay edge positivo en casos similares; debe validarse en shadow antes de cualquier cambio real.",
        }
    return {
